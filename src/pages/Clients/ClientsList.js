...
function ClientsList() {
    ...
    return (
        <div>
            <div className="bg-primary h-44 rounded-b-full sm:rounded-b-2xl lg:rounded-b-2xl "></div>
            <div className="flex justify-center -mt-44 items-end space-x-10  overflow-x-hidden sm:space-x-0">
                {selectedClientIndex !== 0 && (
                    <i
                        className="ri-arrow-left-line text-4xl text-gray-600 cursor-pointer  sm:hidden lg:hidden"
                        onClick={previous}
                    ></i>
                )}

                <div className="grid grid-cols-3 gap-10 sm:grid-cols-1 sm:gap-3 sm:px-5 lg:grid-cols-1">
                    {[
                        clients[selectedClientIndex],
                        clients[selectedClientIndex + 1],
                        clients[selectedClientIndex + 2],
                    ].map((item) => (
                        <div key={item.logo} className="bg-white shadow p-5 w-[400px]  border h-[350px] sm:w-auto sm:h-auto">
                            <div className="flex space-x-10 sm:space-x-0 items-center justify-between sm:flex-col">
                                <img src={item.logo} alt="" className="h-32 w-32 z-0 transform hover:scale-105 duration-300 sm:w-16 sm:h-16 " />
                                <h1 className="text-primary font-semibold text-2xl transform hover:scale-105 duration-300 sm:mt-5">
                                    {item.name}
                                </h1>
                            </div>
                            <p className="text-gray-600 text-md mt-10 transform hover:scale-105 duration-300 sm:mt-5">{item.description}</p>
                        </div>
                    ))}
                </div>
                ...
                {selectedClientIndex < 4 && (
                    <i
                        className="ri-arrow-right-line text-4xl text-gray-600 cursor-pointer sm:hidden  lg:hidden"
                        onClick={next}
                    ></i>
                )}
            </div>
                ...
                <div className="flex space-x-2 ">
                    {[1, 2, 3, 4, 5].map((item, index) => (
                        <div
                            ...
                            className={`bg-primary h-4 w-4 rounded-full cursor-pointer hover:scale-105 transition-all transform duration-300 ${selectedClientIndex === index && "border-2 border-primary bg-secondary h-5 w-5"
                                }`}
                        ></div>
                    ))}
                </div>
